...
dataset = pd.read_csv('adult.data',names=['age','workclass','fnlwgt','education',
                                          'education-num','marital-status',
                                          'occupation','relationship','race','sex',
                                          'capital-gain','capital-loss','hours-per-week',
                                          'native-country','salary'],na_values=' ?')
X = dataset.iloc[:, :-1].values    #only the independent one i.e,0 till n-1
y = dataset.iloc[:, 14].values     #only the dependent i.e, the last one!
#dataset.isnull().sum()

#Taking care of the missing data!
from sklearn.impute import SimpleImputer
imputer=SimpleImputer(missing_values=np.nan,strategy='most_frequent')
imputer=imputer.fit(X[:,[1,6,13]])       #to avoid the dataleakage(testing vs train data)
X[:,[1,6,13]]=imputer.transform(X[:,[1,6,13]])
